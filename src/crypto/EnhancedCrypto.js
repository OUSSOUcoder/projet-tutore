...
export class EnhancedCrypto {
  ...
  /**
   * Génère une paire de clés d'identité RSA-OAEP (pour chiffrer la clé AES de session)
   * Retourne aussi la clé publique au format JWK (facile à transmettre).
   */
  ...
  /**
   * Génère une clé AES-256-GCM pour la session.
   */
  ...
  async initDB() {
    return new Promise((resolve, reject) => {
      // ⚠️ Nouveau nom de DB pour éviter les anciennes sessions incompatibles
      const request = indexedDB.open('CryptoSessionsDB_v3', 1);
  ...
  /**
   * ✅ decryptMessage simplifié pour la simulation :
   * on reconstruit la clé de session AES à chaque fois
   * à partir de encryptedSessionKey + clé privée RSA locale.
   */
  async decryptMessage(senderId, encryptedData, myPrivateKey = null) {
    if (!encryptedData.encryptedSessionKey) {
      throw new Error('encryptedSessionKey manquante');
    }
    if (!myPrivateKey) {
      throw new Error('Clé privée RSA manquante côté destinataire');
    }

    try {
      // 1) Reconstituer la clé AES à partir de la clé de session chiffrée
      const encryptedSessionKey = this.base64ToArrayBuffer(encryptedData.encryptedSessionKey);
      const sessionKeyRaw = await window.crypto.subtle.decrypt(
        { name: 'RSA-OAEP' },
        myPrivateKey,
        encryptedSessionKey
      ...
      // 2) Déchiffrer le message avec cette clé de session éphémère
